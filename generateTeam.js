const fs = require("fs")

function generateTeam(data) {
    var cardHtmls = []
    for (var i = 0; i < data.length; i++) {
        var id = i
        if (data[i].title === "Manager") {
            var managerCard = `
            <div class="card m-3">
            <div class="card-header bg-info text-white font-weight-bold">
                <h1>${data[i].name}</h1>
                <h1> <i class="fas fa-mug-hot"></i>${data[i].title}</h1>
            </div>
            <div class="card-body bg-light" style="color: black;">
                <ul class="list-group">
                    <li class="list-group-item">ID: ${id}</li>
                    <li class="list-group-item">Email: ${data[i].email}</li>
                    <li class="list-group-item">Office number: ${data[i].officenumber}</li>
                </ul>
            </div>
        </div>`
            cardHtmls.push(managerCard)
        } else if (data[i].title === "Intern") {
            var internCard = ` <div class="card m-3">
            <div class="card-header bg-info text-white font-weight-bold">
                <h1>${data[i].name}</h1>
                <h1> <i class="fas fa-mug-hot"></i>${data[i].title}</h1>
            </div>
            <div class="card-body bg-light" style="color: black;">
                <ul class="list-group">
                    <li class="list-group-item">ID: ${id}</li>
                    <li class="list-group-item">Email: ${data[i].email}</li>
                    <li class="list-group-item">School: ${data[i].school}</li>
                </ul>
            </div>
        </div>`
            cardHtmls.push(internCard)
        } else if (data[i].title === "Engineer") {
            var engineerCard = ` <div class="card m-3">
            <div class="card-header bg-info text-white font-weight-bold">
                <h1>${data[i].name}</h1>
                <h1> <i class="fas fa-mug-hot"></i>${data[i].title}</h1>
            </div>
            <div class="card-body bg-light" style="color: black;">
                <ul class="list-group">
                    <li class="list-group-item">ID: ${id}</li>
                    <li class="list-group-item">Email: ${data[i].email}</li>
                    <li class="list-group-item">GitHub: ${data[i].username}</li>
                </ul>
            </div>
        </div>`
            cardHtmls.push(engineerCard)
        }

    }


    const html = `<!DOCTYPE html>
 <html lang="en">
 <head>
 <meta charset="UTF-8">
 <meta name="viewport" content="width=device-width, initial-scale=1.0, shrink-to-fit=no">
 <meta http-equiv="X-UA-Compatible" content="ie=edge">
 <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0/css/bootstrap.min.css"
     integrity="sha384-Gn5384xqQ1aoWXA+058RXPxPg6fy4IWvTNh0E263XmFcJlSAwiGgFAW/dAiS6JXm" crossorigin="anonymous">
 <link rel="stylesheet" href="https://use.fontawesome.com/releases/v5.8.1/css/all.css" />
 <link href="https://fonts.googleapis.com/css?family=BioRhyme|Cabin&display=swap" rel="stylesheet">
 <title>Team</title>
 <style>
     *,
     *::after,
     *::before {
         box-sizing: border-box;
     }
     html,
     body {
         padding: 0;
         margin: 0;
     }
     html,
     body,
     .wrapper {
         height: 100%;
     }
     body {
         background-color: white;
         -webkit-print-color-adjust: exact !important;
         font-family: 'Cabin', sans-serif;
     }
     main {
         background-color: #E9EDEE;
         height: auto;
         padding-top: 30px;
     }
     h1,
     h2,
     h3,
     h4,
     h5,
     h6 {
         font-family: 'BioRhyme', serif;
         margin: 0;
     }
     h2 {
         font-size: 2.5em;
     }
     h6 {
         font-size: 1.2em;
     }
     .container {
         padding: 50px;
     }
     .row {
         display: flex;
         flex-wrap: wrap;
         justify-content: space-between;
         margin-top: 00px;
         margin-bottom: 0px;
     }
     .card {
        height: 351px;
        width: 315px;
        padding: 20px;
        border-radius: 6px;
        background-color: grey;
    }
     li {
         background-color: grey;
     }
     .col {
         flex: 1;
         text-align: center;
     }
     .teamheader {
        text-align: center;
        background-color: rgba(0, 54, 124, 0.795);
    }
 </style>
 </head>
 <body>
 <div class="container">
    <div class="teamheader bg-info" style="text-align: center;">
        <h1>
            The A Team!
        </h1>
    </div>
    <div class="row">
        ${cardHtmls}
    </div>
</div>
 </body>
 </html>`
    console.log("this is team data", data)
    fs.writeFile("team.html", html, function (error) {
        console.log(error)
    })

}

module.exports = generateTeam 